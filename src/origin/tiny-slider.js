...
export var tns = function(options) {
  ...
  var items = getOption('items'),
      ...
      /**
       * 当循环轮播的时候，如果图片总数小于或等于每页展示的图片数的话，则轮播滚动失效了。原因是这里的判断将freeze设为true了。修改一下
       * by yangxu 2018.1.18
       */
      // freeze = disable ? true : freezable ? slideCount <= items : false,
      freeze = disable ? true : freezable ? (!loop && slideCount <= items) : false,
      /** end */
      frozen,
  ...
  function getOption (item, viewport) {
    ...
    if (!carousel && item in obj) {
      ...
      if (item === 'items' && getOption('fixedWidth')) {
        ...
        /**
         * 在设置了fixedWidth属性等于轮播图片的宽度时，如果也设置了gutter属性大于0，此处获取的vpOuter是等于轮播图片的宽度的，因此会出现result小于1的情况，再进行floor处理，就变成0了。导致图片加载不出来
         * 此处做个修改
         * by yangxu 2018.1.2
         */
        if (result < 1) {
...
// === ON RESIZE ===
      ...
      for(; i < len; i++) {
        ...
        [].forEach.call(slideItems[i].querySelectorAll('.tns-lazy-img'), function (img) {
          ...
          if (!hasClass(img, 'loaded')) {
            /**
             * 由于轮播的图片尺寸不一，此处使用div的backgroundImage属性来加载图片会方便设置按照指定尺寸居中显示图片，因此修改一下
             * by yangxu 2018.1.4
             */
            // img.src = getAttr(img, 'data-src');
            img.style.backgroundImage = `url(${encodeURI(getAttr(img, 'data-src'))})`;
            /** end */
            addClass(img, 'loaded');
          }
        });
    ...
    // go to exact slide
      ...
      if (targetIndex === 'first') {
        ...
        if (!isNaN(targetIndex)) {
          ...
          /**
           * 当循环且从上一组的索引直接跳到下一组的索引时，会有目标真实索引absTargetIndex小于原真实索引absIndex的情况，导致索引偏移量indexGap小于0，轮播的动画产生向回播放的效果
           * 此处做个修改
           * by yangxu 2017.12.27
           */
          if (loop) {
  ...
  return {
    ...
    destroy: function () {
      /**
       * IE11destroy报错，最外层加个try...catch...
       * by yangxu 2018.1.19
       */
      try {
        /**
         * 重复调用destroy的时候报错，加个判断
         * by yangxu 2018.6.12
         */
